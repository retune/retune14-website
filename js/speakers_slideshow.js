var d = 3000;

$("#allen_image").backstretch([
  "img/speakers/allen_03.jpg",
  "img/speakers/allen_02.jpg",
  "img/speakers/allen_01.jpg"
], { fade: 750, duration: d });

$("#allen_small").backstretch([
  "img/speakers/allen_03.jpg",
  "img/speakers/allen_02.jpg",
  "img/speakers/allen_01.jpg"
], { fade: 750, duration: d });

$("#smigla-bobinski_image").backstretch([
  "img/speakers/smigla-bobinski_01.jpg",
  "img/speakers/smigla-bobinski_02.jpg",
  "img/speakers/smigla-bobinski_03.jpg",
  "img/speakers/smigla-bobinski_04.jpg",
  "img/speakers/smigla-bobinski_05.jpg",
  "img/speakers/smigla-bobinski_06.jpg"
], { fade: 750, duration: d });

$("#smigla-bobinski_small").backstretch([
  "img/speakers/smigla-bobinski_01.jpg",
  "img/speakers/smigla-bobinski_02.jpg",
  "img/speakers/smigla-bobinski_03.jpg",
  "img/speakers/smigla-bobinski_04.jpg",
  "img/speakers/smigla-bobinski_05.jpg",
  "img/speakers/smigla-bobinski_06.jpg"
], { fade: 750, duration: d });

$("#franco_image").backstretch([
  "img/speakers/franco_01.jpg",
  "img/speakers/franco_02.jpg",
  "img/speakers/franco_03.jpg",
  "img/speakers/franco_04.jpg"
], { fade: 750, duration: d });

$("#franco_small").backstretch([
  "img/speakers/franco_01.jpg",
  "img/speakers/franco_02.jpg",
  "img/speakers/franco_03.jpg",
  "img/speakers/franco_04.jpg"
], { fade: 750, duration: d });

$("#sauter_image").backstretch([
  "img/speakers/sauter_01.jpg",
  "img/speakers/sauter_02.jpg",
  "img/speakers/sauter_03.jpg",
  "img/speakers/sauter_04.jpg",
  "img/speakers/sauter_05.jpg",
  "img/speakers/sauter_06.jpg",
  "img/speakers/sauter_07.jpg"
], { fade: 750, duration: d });

$("#sauter_small").backstretch([
  "img/speakers/sauter_01.jpg",
  "img/speakers/sauter_02.jpg",
  "img/speakers/sauter_03.jpg",
  "img/speakers/sauter_04.jpg",
  "img/speakers/sauter_05.jpg",
  "img/speakers/sauter_06.jpg",
  "img/speakers/sauter_07.jpg"
], { fade: 750, duration: d });

$("#quaranta_image").backstretch([
  "img/speakers/quaranta_01.jpg",
  "img/speakers/quaranta_02.jpg"
], { fade: 750, duration: d });

$("#quaranta_small").backstretch([
  "img/speakers/quaranta_01.jpg",
  "img/speakers/quaranta_02.jpg"
], { fade: 750, duration: d });

$("#random_image").backstretch([
  "img/speakers/random_01.jpg",
  "img/speakers/random_02.jpg",
  "img/speakers/random_03.jpg",
  "img/speakers/random_04.jpg",
  "img/speakers/random_05.jpg",
  "img/speakers/random_06.jpg"
], { fade: 750, duration: d });

$("#random_small").backstretch([
  "img/speakers/random_01.jpg",
  "img/speakers/random_02.jpg",
  "img/speakers/random_03.jpg",
  "img/speakers/random_04.jpg",
  "img/speakers/random_05.jpg",
  "img/speakers/random_06.jpg"
], { fade: 750, duration: d });

$("#uva_image").backstretch([
  "img/speakers/uva_01.jpg",
  "img/speakers/uva_02.jpg",
  "img/speakers/uva_03.jpg",
  "img/speakers/uva_04.jpg"
], { fade: 750, duration: d });

$("#uva_small").backstretch([
  "img/speakers/uva_01.jpg",
  "img/speakers/uva_02.jpg",
  "img/speakers/uva_03.jpg",
  "img/speakers/uva_04.jpg"
], { fade: 750, duration: d });

$("#aparede_image").backstretch([
  "img/speakers/aparede_01.jpg",
  "img/speakers/aparede_02.jpg",
  "img/speakers/aparede_03.jpg"
], { fade: 750, duration: d });

$("#aparede_small").backstretch([
  "img/speakers/aparede_01.jpg",
  "img/speakers/aparede_02.jpg",
  "img/speakers/aparede_03.jpg"
], { fade: 750, duration: d });

$("#blas_image").backstretch([
  "img/speakers/blas_01.jpg",
  "img/speakers/blas_02.jpg",
  "img/speakers/blas_03.jpg"
], { fade: 750, duration: d });

$("#blas_small").backstretch([
  "img/speakers/blas_01.jpg",
  "img/speakers/blas_02.jpg",
  "img/speakers/blas_03.jpg"
], { fade: 750, duration: d });

$("#gohl_image").backstretch([
  "img/speakers/gohl_01.jpg",
  "img/speakers/gohl_02.jpg",
  "img/speakers/gohl_03.jpg",
  "img/speakers/gohl_04.jpg"
], { fade: 750, duration: d });

$("#gohl_small").backstretch([
  "img/speakers/gohl_01.jpg",
  "img/speakers/gohl_02.jpg",
  "img/speakers/gohl_03.jpg",
  "img/speakers/gohl_04.jpg"
], { fade: 750, duration: d });

$("#wang_image").backstretch([
  "img/speakers/wang_01.jpg",
  "img/speakers/wang_02.jpg",
  "img/speakers/wang_03.jpg",
  "img/speakers/wang_04.jpg"
], { fade: 750, duration: d });

$("#wang_small").backstretch([
  "img/speakers/wang_01.jpg",
  "img/speakers/wang_02.jpg",
  "img/speakers/wang_03.jpg",
  "img/speakers/wang_04.jpg"
], { fade: 750, duration: d });

$("#visnjic_image").backstretch([
  "img/speakers/visnjic_01.jpg",
  "img/speakers/visnjic_02.jpg",
  "img/speakers/visnjic_03.jpg"
], { fade: 750, duration: d });

$("#visnjic_small").backstretch([
  "img/speakers/visnjic_01.jpg",
  "img/speakers/visnjic_02.jpg",
  "img/speakers/visnjic_03.jpg"
], { fade: 750, duration: d });

$("#schmidt_image").backstretch([
  "img/speakers/schmidt_01.jpg",
  "img/speakers/schmidt_02.jpg",
  "img/speakers/schmidt_03.jpg"
], { fade: 750, duration: d });

$("#schmidt_small").backstretch([
  "img/speakers/schmidt_01.jpg",
  "img/speakers/schmidt_02.jpg",
  "img/speakers/schmidt_03.jpg"
], { fade: 750, duration: d });

$("#mirti_image").backstretch([
  "img/speakers/mirti_01.jpg",
  "img/speakers/mirti_02.jpg",
  "img/speakers/mirti_03.jpg",
  "img/speakers/mirti_04.jpg",
  "img/speakers/mirti_05.jpg"
], { fade: 750, duration: d });

$("#mirti_small").backstretch([
  "img/speakers/mirti_01.jpg",
  "img/speakers/mirti_02.jpg",
  "img/speakers/mirti_03.jpg",
  "img/speakers/mirti_04.jpg",
  "img/speakers/mirti_05.jpg"
], { fade: 750, duration: d });

$("#cyborgs_image").backstretch([
  "img/speakers/cyborgs_01.jpg",
  "img/speakers/cyborgs_02.jpg"
], { fade: 750, duration: d });

$("#cyborgs_small").backstretch([
  "img/speakers/cyborgs_01.jpg",
  "img/speakers/cyborgs_02.jpg"
], { fade: 750, duration: d });

$("#shalom_image").backstretch([
  "img/speakers/shalom_01.jpg",
  "img/speakers/shalom_02.jpg",
  "img/speakers/shalom_03.jpg"
], { fade: 750, duration: d });

$("#shalom_small").backstretch([
  "img/speakers/shalom_01.jpg",
  "img/speakers/shalom_02.jpg",
  "img/speakers/shalom_03.jpg"
], { fade: 750, duration: d });

$("#geist_image").backstretch([
  "img/speakers/geist_01.jpg",
  "img/speakers/geist_02.jpg",
  "img/speakers/geist_03.jpg",
  "img/speakers/geist_04.jpg"
], { fade: 750, duration: d });

$("#geist_small").backstretch([
  "img/speakers/geist_01.jpg",
  "img/speakers/geist_02.jpg",
  "img/speakers/geist_03.jpg",
  "img/speakers/geist_04.jpg"
], { fade: 750, duration: d });

$("#parmar_image").backstretch([
  "img/speakers/parmar_01.jpg"
], { fade: 750, duration: d });

$("#parmar_small").backstretch([
  "img/speakers/parmar_01.jpg"
], { fade: 750, duration: d });

$("#lieser_image").backstretch([
  "img/speakers/lieser_01.jpg",
  "img/speakers/lieser_02.jpg",
  "img/speakers/lieser_03.jpg",
  "img/speakers/lieser_04.jpg",
  "img/speakers/lieser_05.jpg"
], { fade: 750, duration: d });

$("#lieser_small").backstretch([
  "img/speakers/lieser_01.jpg",
  "img/speakers/lieser_02.jpg",
  "img/speakers/lieser_03.jpg",
  "img/speakers/lieser_04.jpg",
  "img/speakers/lieser_05.jpg"
], { fade: 750, duration: d });

$("#beanotherlab_image").backstretch([
  "img/speakers/beanotherlab_01.jpg",
  "img/speakers/beanotherlab_02.jpg",
  "img/speakers/beanotherlab_03.jpg",
  "img/speakers/beanotherlab_04.jpg",
  "img/speakers/beanotherlab_05.jpg",
  "img/speakers/beanotherlab_06.jpg"
], { fade: 750, duration: d });

$("#beanotherlab_small").backstretch([
  "img/speakers/beanotherlab_01.jpg",
  "img/speakers/beanotherlab_02.jpg",
  "img/speakers/beanotherlab_03.jpg",
  "img/speakers/beanotherlab_04.jpg",
  "img/speakers/beanotherlab_05.jpg",
  "img/speakers/beanotherlab_06.jpg"
], { fade: 750, duration: d });

$("#dubois_image").backstretch([
  "img/speakers/dubois_01.jpg",
  "img/speakers/dubois_02.jpg",
  "img/speakers/dubois_03.jpg",
  "img/speakers/dubois_04.jpg"
], { fade: 750, duration: d });

$("#dubois_small").backstretch([
  "img/speakers/dubois_01.jpg",
  "img/speakers/dubois_02.jpg",
  "img/speakers/dubois_03.jpg",
  "img/speakers/dubois_04.jpg"
], { fade: 750, duration: d });

// pre-load images
if(document.images){
  // var image1 = new Image()        
 //   image1.src = "http://retune.de/2013/img/Auger_Blinddate.jpg"
 //   var image2 = new Image()
 //   image2.src = "http://retune.de/2013/img/KyleMcDonald_LightLeaks.jpg"
}